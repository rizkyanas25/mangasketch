...
vi.mock('../services/aiService', () => {
  return {
    AiService: {
      generateMangaPanel: vi
        .fn()
        .mockImplementation(async (prompt: string, seed?: number) => {
...
vi.mock('../services/sketchService', () => {
  return {
    SketchService: {
      ...
      getSketchWithHistory: vi
        .fn()
        .mockImplementation(async (sketchId: string, userId: string) => {
      ...
      uploadSketchToStorage: vi
        .fn()
        .mockImplementation(async (buffer: Buffer, filepath: string) => {
          return 'http://example.com/recovered-sketch.webp';
        }),
      saveSketchToDatabase: vi
        .fn()
        .mockImplementation(async (sketchData: any) => {
          return {
...
describe('Sketches API Routes', () => {
    ...
    it('should save pre-generated anonymous sketch without AI call if imageUrl is provided and auth is valid', async () => {
      const res = await request(app)
        ...
        .send({
          ...
          drawingStyle: 'INKED_MANGA',
          imageUrl:
            'data:image/webp;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==',
          seed: 99,
        });
      ...
      expect(res.body.imageUrl).toBe(
        'http://example.com/recovered-sketch.webp',
      );
  ...
  describe('GET /api/sketches', () => {
    ...
    it('should return 200 and sketches list for authenticated user', async () => {
      ...
      expect(SketchService.getUserSketches).toHaveBeenCalledWith(
        'mock-user-id',
      );
    });
  });
  ...
  describe('GET /api/sketches/:id', () => {
    it('should return 401 if user is not authenticated', async () => {
      const res = await request(app).get(
        '/api/sketches/11111111-1111-1111-1111-111111111111',
      );
      expect(res.status).toBe(401);
      expect(res.body.code).toBe('UNAUTHORIZED');
      expect(res.body.message).toContain('logged in');
    ...
    it('should return 200 and sketch history for authenticated user and valid UUID', async () => {
      ...
      expect(SketchService.getSketchWithHistory).toHaveBeenCalledWith(
        targetId,
        'mock-user-id',
      );
    });
  ...
  describe('DELETE /api/sketches/:id', () => {
    ...
    it('should return 200 and success status on successful deletion', async () => {
      ...
      expect(SketchService.deleteSketch).toHaveBeenCalledWith(
        targetId,
        'mock-user-id',
      );
    });
  ...
  describe('Rate Limiting (Ink Quota) enforcement', () => {
    it('should enforce anonymous limit of 5 requests and return 429 when exhausted', async () => {
      ...
      const res429 = await request(app).post('/api/sketches').send({
