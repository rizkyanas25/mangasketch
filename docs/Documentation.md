...
User Browser                  Express Backend                AI API & Supabase
     │                               │                                 │
     ├── 1. User types prompt & selects manga style                    │
     ├── 2. POST /api/generate ─────>│                                 │
     │      Body: { prompt, style }  │                                 │
     │      Header: Bearer <jwt>     ├── 3. Validate prompt (not empty)│
     │                               ├── 4. Build prompt via template  │
     │                               │      "manga sketch, B&W..."     │
     │                               ├── 5. GET /prompt/{prompt} ─────>│ [Pollinations.ai]
     │                               │      (Wait 10-30s for image)    │
     │                               |<── 6. Receive image binary ─────│
     │                               ├── 7. [Is Authenticated?]        │
     │                               │      ├── YES: Upload PNG ──────>│ [Supabase Storage]
     │                               │      │        Save metadata ───>│ [PostgreSQL DB]
     │                               │      └── NO: Return base64 URL  │
     │<── 8. Return response payload─│                                 │
     ▼                               ▼                                 ▼
[Display Manga Sketch]       [Finish Generation]              [Secured / Transient]
...
- **Original Plan:** Rely on system-level font installations (e.g., expecting _Noto Sans JP_ and _Impact_ to be pre-installed on the host operating system) or inline Base64 WOFF2 font embedding inside the SVG.
...
### 12. Limited HTTP Request/Response Payload Logging in Railway

- **Current Limitation:** Railway's default HTTP log tab (metadata routing proxy) only captures network metadata (such as HTTP method, URL path, status code, duration, and bytes sent/received). It does not capture or store the request payload (HTTP body) or response body.
- **Impact:** Debugging API failures, tracking active user prompts, or verifying quota updates in real-time is difficult. To address this, a custom HTTP request/response logging middleware was implemented to output logs to Railway's "Deploy Logs" stream. However, since guest users receive generated images as large base64 data URLs, logging the entire response body directly would freeze the dashboard and inflate log usage. The middleware solves this by dynamically sanitizing and truncating base64 data URLs and long strings.
- **Production-Grade Solution:** The logging middleware is guarded by the `DEBUG_HTTP=true` environment variable, allowing it to be disabled in production to protect user privacy and preserve I/O performance. For a production environment, the best practice is to turn off this custom middleware and instead integrate the application with specialized log management platforms (such as Datadog, Axiom, or Better Stack Logtail). These specialized platforms provide secure dashboards to ingest, filter, and index JSON payloads without affecting the core server's resources.
